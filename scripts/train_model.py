...
Split: train on 2021-2024, hold out all of 2025 as the test set. A full-season
holdout (not a row-count 80/20 split) avoids cutting a season in half and
keeps the evaluation honest -- the model never sees any 2025 result during
training, mirroring how it would actually be used (predict a season it
hasn't seen yet).
"""
import sys
...
def main():
    ...
    print(f"Train: {len(train_df)} games (2021-{HOLDOUT_YEAR - 1}). Test: {len(test_df)} games ({HOLDOUT_YEAR}).")
    ...
    bundle = {
        ...
        "feature_medians": medians, "train_years": (2021, HOLDOUT_YEAR - 1), "holdout_year": HOLDOUT_YEAR,
        "classifier_metrics": class_metrics, "regressor_metrics": reg_metrics,
    }
